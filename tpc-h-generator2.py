...
def main():
    ...
    res = Set()
    for group in groups:
        sum_base_price = {item.l_extendedprice for item in resFull if ((item.l_returnflag == group[0]) and (item.l_linestatus == group[1]))}
        sum_base_price = sum(sum_base_price)
        sum_qty = {item.l_quantity for item in resFull if ((item.l_returnflag == group[0]) and (item.l_linestatus == group[1]))}
        sum_qty = sum(sum_qty)
        sum_disc_price = {(item.l_extendedprice * (1.0 - item.l_discount)) for item in resFull if ((item.l_returnflag == group[0]) and (item.l_linestatus == group[1]))}
        sum_disc_price = sum(sum_disc_price)
        sum_charge = {((item.l_extendedprice * (1.0 - item.l_discount)) * (1.0 + item.l_tax)) for item in resFull if ((item.l_returnflag == group[0]) and (item.l_linestatus == group[1]))}
        sum_charge = sum(sum_charge)
        count = {item for item in resFull if ((item.l_returnflag == group[0]) and (item.l_linestatus == group[1]))}
        count = count(count)
        avg_qty = (sum_qty / count)
        avg_price = (sum_base_price / count)
        avg_disc = {item.l_discount for item in resFull if ((item.l_returnflag == group[0]) and (item.l_linestatus == group[1]))}
